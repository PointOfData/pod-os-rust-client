...
impl Retry {
    pub fn new(
        ...
        Self { retries, backoff, backoff_multiplier, disable_backoff_caps }
    }
                ...
                Err(e) => {
                    ...
                    if last {
                        return Err(GatewayDError::new(
                            ...
                            format!("retries exhausted after {} attempts: {}", attempt + 1, e.message),
                        ));
                    }
...
fn apply_caps(secs: f64, disabled: bool) -> f64 {
    if disabled { secs } else { secs.min(BACKOFF_DURATION_CAP.as_secs_f64()) }
}
...
mod tests {
    ...
    async fn exhausted_does_not_panic() {
        let r = Retry { retries: 2, backoff: Duration::from_millis(1), backoff_multiplier: 1.0, disable_backoff_caps: true };
        let res: Result<(), _> = r.run(|_| async {
            Err(GatewayDError::new(ErrCode::Unknown, "always fail"))
        }).await;
        assert!(res.is_err());
    }
    ...
    async fn succeeds_after_failures() {
        let r = Retry { retries: 5, backoff: Duration::from_millis(1), backoff_multiplier: 1.0, disable_backoff_caps: true };
        ...
        let res = r.run(|_| {
            ...
            async move {
                ...
                if n < 3 { Err(GatewayDError::new(ErrCode::Unknown, "fail")) } else { Ok(n) }
            }
        }).await;
        assert!(res.is_ok());
    }
