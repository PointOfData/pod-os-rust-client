...
pub fn resolve(network: &str, address: &str) -> Result<String, GatewayDError> {
    if TCP_NETWORKS.contains(&network) || UDP_NETWORKS.contains(&network) {
        ...
        panic!("pod-os-client: unsupported network '{}' — expected tcp/udp/unix", network);
    }
}
...
fn resolve_tcp_udp(address: &str) -> Result<String, GatewayDError> {
    ...
    let mut addrs = address.to_socket_addrs().map_err(|e| {
        GatewayDError::wrap(ErrCode::ResolveFailed, format!("cannot resolve '{}'", address), e)
    })?;
    addrs.next()
        .map(|a| a.to_string())
        .ok_or_else(|| GatewayDError::new(ErrCode::ResolveFailed, format!("no addresses for '{}'", address)))
}
