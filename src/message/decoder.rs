...
pub fn decode_message(raw: &[u8]) -> Result<Message, DecodeError> {
    ...
    let mut pos = 63usize;
    let to_bytes = &raw[pos..pos + to_len];
    pos += to_len;
    let from_bytes = &raw[pos..pos + from_len];
    pos += from_len;
    let header_bytes = &raw[pos..pos + header_len];
    pos += header_len;
...
fn parse_dec_field(field: &[u8], name: &str) -> Result<i32, DecodeError> {
    let s = std::str::from_utf8(field).map_err(|_| {
        DecodeError::new(
            MsgErrCode::DecodeInvalidMessageType,
            format!("{name}: non-UTF8"),
        )
    })?;
    s.trim_start_matches('0')
...
fn decode_event_field(key: &str, val: &str, event: &mut EventFields) {
    match key {
        ...
        "unique_id" | "_unique_id" | "tag:1:_unique_id" => event.unique_id = val.to_string(),
        ...
        if let Some(_rest) = line.strip_prefix("_event_id=") {
            ...
            if !parts.is_empty() {
                brief_hits.push(BriefHitRecord {
                    ...
                    total_hits: parts
                        .get(1)
                        .and_then(|s| s.strip_prefix("_total_hits="))
    ...
    for line in payload.lines() {
        ...
        for (k, v) in &fields {
            match k.as_str() {
                ...
                "_msg" => {
                    record.message = v.clone();
                    continue;
                }
                "_count" => {
                    record.event_count = v.parse().unwrap_or(0);
                    continue;
                }
                _ => {}
            }
        ...
        for (k, v) in &fields {
            match k.as_str() {
                ...
                "total_link_requests_found" => {
                    record.total_link_requests_found = v.parse().unwrap_or(0)
                }
                "_links_ok" => {
                    record.links_ok = v.parse().unwrap_or(0);
                    is_summary = true;
                }
