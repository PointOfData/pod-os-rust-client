...
impl Intent {
    const fn new(
        ...
        Self { name, routing_message_type, neural_memory_command, message_type }
    }
    pub fn is_zero(&self) -> bool { self.name.is_empty() }
}

// ── All known intents ────────────────────────────────────────────────────────

pub static STORE_EVENT:           Intent = Intent::new("StoreEvent",           "MEM_REQ",   "store",           1000);
pub static STORE_BATCH_EVENTS:    Intent = Intent::new("StoreBatchEvents",     "MEM_REQ",   "store_batch",     1000);
pub static STORE_BATCH_TAGS:      Intent = Intent::new("StoreBatchTags",       "MEM_REQ",   "tag_store_batch", 1000);
pub static GET_EVENT:             Intent = Intent::new("GetEvent",             "MEM_REQ",   "get",             1000);
pub static GET_EVENTS_FOR_TAGS:   Intent = Intent::new("GetEventsForTags",     "MEM_REQ",   "events_for_tag",  1000);
...
pub static STORE_BATCH_LINKS:     Intent = Intent::new("StoreBatchLinks",      "MEM_REQ",   "link_batch",      1000);

pub static STORE_EVENT_RESPONSE:        Intent = Intent::new("StoreEventResponse",        "MEM_REPLY", "store",           1001);
pub static STORE_BATCH_EVENTS_RESPONSE: Intent = Intent::new("StoreBatchEventsResponse",  "MEM_REPLY", "store_batch",     1001);
pub static STORE_BATCH_TAGS_RESPONSE:   Intent = Intent::new("StoreBatchTagsResponse",    "MEM_REPLY", "tag_store_batch", 1001);
pub static GET_EVENT_RESPONSE:          Intent = Intent::new("GetEventResponse",          "MEM_REPLY", "get",             1001);
pub static GET_EVENTS_FOR_TAGS_RESPONSE: Intent = Intent::new("GetEventsForTagsResponse","MEM_REPLY", "events_for_tag",  1001);
pub static LINK_EVENT_RESPONSE:         Intent = Intent::new("LinkEventResponse",         "MEM_REPLY", "link",            1001);
pub static UNLINK_EVENT_RESPONSE:       Intent = Intent::new("UnlinkEventResponse",       "MEM_REPLY", "unlink",          1001);
pub static STORE_BATCH_LINKS_RESPONSE:  Intent = Intent::new("StoreBatchLinksResponse",   "MEM_REPLY", "link_batch",      1001);
...
const ALL_INTENTS: &[&Intent] = &[
    ...
    &ACTOR_ECHO, &ACTOR_START, &STATUS, &ACTOR_REQUEST, &ACTOR_RESPONSE,
